...
struct regNo
{
    ...
    struct regNo *mae; // Ponteiro para o nó pai (usado para facilitar a remoção)
    struct regNo *dir; // Ponteiro para o filho à direita (maiores)
};
typedef struct regNo TNo;

// Protótipos das funções
int PesquisaValor(TNo *, int);              // Pesquisa se um valor existe
TNo *Remove(TNo *, int);                    // Remove um nó da árvore
TNo *AchaPai(TNo *, int);                   // Encontra o nó pai para inserção
void ImprimeArvorePre(TNo *r, int *primeiro); // Impressão Pré-ordem
void ImprimeArvoreIn(TNo *r, int *primeiro);  // Impressão Em-ordem (Infixa)
void ImprimeArvorePost(TNo *r, int *primeiro);// Impressão Pós-ordem
